...
describe('grouped event setup view', () => {
  ...
  it('keeps view cards read-only and edits categories with icon entry deletion', async () => {
    ...
    expect(screen.getByLabelText('Category title')).toHaveClass('app-text-input')
    expect(screen.getByLabelText('Category title').closest('.form-row')).toHaveClass('form-row-full')
    expect(screen.getByLabelText('Entry title for Peyton')).toHaveClass('app-text-input')
    expect(screen.getByRole('list', { name: 'Desserts entries' })).toHaveClass('category-entry-edit-list')
